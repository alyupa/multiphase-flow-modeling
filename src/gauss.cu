...
//obraschenie matrici
__device__ int device_reverse_matrix (double *a, int n)
{
	int i, j, k;
	double f, f1;
	double *matr, *row;
	...
	for (k = 0; k < n; k++)
	{
		...
		if (fabs(f) < e)
		{
			...
			if (det_flag == 0)
			{
				printf("Determinant = 0 in reverse_matrix!\n");
				return -1;
			}	
		}
...
__device__ void device_mult_matrix_vector (double* result_vect, double* matr, double* vect, int n) 
{
	for (int i = 0; i < n; i++)
	{
		result_vect[i] = 0;
